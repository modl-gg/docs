...
| `type` | string | Yes | The ticket type. One of: `player`, `player_report`, `chat`, `chat_report`, `bug`, `bug_report`, `appeal`, `ban_appeal`, `application`, `staff`, `staff_application`, `apply`, `support`, `general_support`. |
...
| `priority` | string | No | The ticket priority. One of: `low`, `minor`, `normal`, `medium`, `default`, `standard`, `high`, `urgent`, `critical`, `highest`. Defaults to `normal`. |
| `createdServer` | string | No | The name of the server where the ticket was created. |
| `replayUrl` | string | No | A URL to a replay file associated with the ticket. |
